...
**AI 辅助量化投研的 fail-closed 证据清单（evidence manifest）。**

这不是因子库、回测引擎或交易系统。`holdout-governance` 做的是金融 AI
Agent 自动生成投研结论、调用工具、挂载证据之后的基础设施：这个产物能不能
进入下一步，缺什么证据，哪里必须人工确认。
...
`holdout-governance` 是 AI 辅助投研工作流外面的一层边界。AI 可以生成结论、调用工具、挂载证据，但能不能进入发布和决策链，要由清单和策略文件决定。证据缺失默认拦截；告警能否降级，只看 `policy.yml`；最终发布仍交给人或外部发布流程。

这条契约的意思是：AI 可以帮你做研究，但不能静默跳过证据、抹掉责任链，也不能自己把草稿变成已发布结论。

这也是 Holdout 和常见量化工具不同的地方。常见工具帮你找信号、做回测、
组合优化或执行交易；Holdout 管的是这些动作外面的研究产物：用了什么证据、
过了哪些检查、套用了什么规则、哪里需要人来批准。
